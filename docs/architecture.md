...
| Notifications | Postgres rows, plus a realtime ping | One row per person told about one thing, written on the caller's connection so it commits with the change it announces. The bell refetches on the ping. See [notifications.md](notifications.md) |
| Audit log | Append-only Postgres rows | One row per recorded act, written on the caller's connection so it commits with the write that caused it. Framework surfaces record themselves, and scaffolded models ride the seams webhooks emit from. See [audit.md](audit.md) |
| Passwords | argon2id | |
...
- `anubis/` is a single Cargo package. It is both the framework library and the `anubis` CLI binary (`cargo install anubis-framework` provides the CLI). The package publishes as `anubis-framework`, because an unrelated crate holds the plain name on crates.io; the library, the binary, and the dependency line an application writes are all `anubis`, through Cargo's dependency renaming (`anubis = { package = "anubis-framework", version = "..." }`). See [ci.md](ci.md#the-crate-name). It declares no cargo features: webhooks, jobs, realtime, and billing are all part of the library, and each one costs nothing until an application mounts it. Features are reserved for the day a subsystem carries a dependency an application should be able to refuse; every feature would otherwise be an additive one that is always on.
...
Paths under the framework's own prefixes (`/account`, `/api`, `/auth`, `/billing`, `/developers`, `/realtime`, `/tenancy`, `/users`, `/webhooks`) are the exception: an unmatched path there answers the API's JSON `404` rather than the SPA, because HTML with a `200` turns a routing mistake into a parse error far from its cause. `/account` is on that list because the framework serves the notification inbox there, and it is where an application mounts its own account routes. Applications reserve any further prefixes of their own the same way.
