...
1. Use `rails generate super_scaffold` to scaffold a new model:
    ...
    In this example, `Team` refers to the immediate parent of the `Project` resource. For more details, just run `rails generate super_scaffold` or [read the documentation](super-scaffolding.md).
...
2. Use `rails generate super_scaffold:field` to add a new field to a model you've already scaffolded:
    ...
    These first two points about Super Scaffolding are just the tip of the iceberg, so be sure to circle around and [read the full documentation](super-scaffolding.md).
