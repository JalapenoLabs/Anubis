...
**NOTE** Super Scaffolding generates code that contains special "magic comments" that we use as targets for later Super Scaffolding operations. You shouldn't remove those comments. See [Targets for Super Scaffolding](super-scaffolding/targets.md) for more details.
...
Before getting started with Super Scaffolding, we recommend reading about [the philosophy of domain modeling in Bullet Train](modeling.md).
...
The following commands are for use specifically with [Action Models](action-models.md).
...
In the above example, `team` represents the model that a `Project` primarily belongs to. Also, `text_field` was selected from [the list of available field partials](field-partials.md). We'll show examples with `trix_editor` and `super_select` later.
...
We have multiple [field partials](field-partials.md) that we could use for this purpose, including `buttons`, `options`, or `super_select`.
...
## Advanced Examples
 - [Super Scaffolding Options](super-scaffolding/options.md)
 - [Super Scaffolding with Delegated Types](super-scaffolding/delegated-types.md)
 - [Super Scaffolding with the `--sortable` option](super-scaffolding/sortable.md)

## Additional Info

 - [Targets for Super Scaffolding](super-scaffolding/targets.md)
