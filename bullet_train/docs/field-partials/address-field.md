...
The `address_field` partial implements Bullet Train's [Dependent Fields Pattern](dynamic-forms-dependent-fields.md) to automatically update the `region_id` and `postal_code` fields and their labels based on the value of the selected `country_id`.
...
See the [Showcase preview](https://github.com/bullet-train-co/showcase) for the example output.